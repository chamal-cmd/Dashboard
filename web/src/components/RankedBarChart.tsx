      ...
      {sorted.length === 0 ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {sorted.map((r) => {
            ...
            const pct = (v / max) * 100;
            return (
              <div key={r.id ?? r.label} style={{ display: "flex", alignItems: "center", gap: 12 }} title={`${r.label}: ${v}${metric.unit ?? ""}`}>
                <div style={{ width: 150, flexShrink: 0, fontSize: 13, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                  {r.href ? (
                    <Link href={r.href} className="dpDrillBtn" style={{ maxWidth: "100%" }}>{r.label}</Link>
                  ) : (
                    <span style={{ color: "var(--text-1)", fontWeight: 500 }}>{r.label}</span>
                  )}
                </div>
                <div style={{ flex: 1, background: "var(--surface-2)", borderRadius: 4, height: 22, position: "relative", minWidth: 0 }}>
                  <div style={{ position: "absolute", left: 0, top: 0, height: "100%", width: `${Math.max(pct, 1.5)}%`, background: metric.color, borderRadius: 4 }} />
                </div>
                <div style={{ width: 52, flexShrink: 0, textAlign: "right", fontSize: 13, fontWeight: 700, color: "var(--text-2)" }}>
                  {v}{metric.unit ?? ""}
                </div>
              </div>
            );
