...
async function hGet(path: string, key: string, attempt = 0): Promise<unknown> {
  ...
  if (res.status === 429) {
    ...
    throw new Error(`Rate limited after retries: ${path}`);
  }
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const inboxesWithUsers = inboxes as (typeof inboxes[number] & { _userIds?: number[] })[];

    for (const inbox of inboxesWithUsers) {
      // users + tags in parallel (small payloads, low page counts)
      const [us, ts] = await Promise.all([
        hAll(`v1/inboxes/${inbox.id}/users?limit=100`, key),
        hAll(`v1/inboxes/${inbox.id}/tags?limit=100`, key),
      ]);
      inbox._userIds = (us as { id: number }[]).map((u) => u.id);
    ...
    return NextResponse.json({ inboxes, users, tags, conversations });
  } catch (e) {
